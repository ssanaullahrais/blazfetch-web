...
| `VITE_HIDE_INCOMPATIBLE_VIDEO_ON_PHONE` | on | Hides a video format flagged "may not play everywhere" (VP9/AV1/etc.) from the list on phones and tablets, where many devices can't decode it at all; desktop always sees every format. `false` shows every format everywhere. A source with nothing compatible left is shown as-is rather than an empty list |
