...
| `VITE_HIDE_INCOMPATIBLE_VIDEO_ON_PHONE` | on | Hides a video format flagged "may not play everywhere" (VP9/AV1/etc.) from the list on a narrow, phone-width screen (under 768px), where many devices can't decode it at all; a wide (desktop) screen always sees every format. `false` shows every format at every width. A source with nothing compatible left is shown as-is rather than an empty list |
